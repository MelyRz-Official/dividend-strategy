        ...
        for stock in dividend_stocks:
            ...
            try:
                ...
                if dividend_data and dividend_data.get('dividend_yield', 0) > 0:
                    ...
                    print(f"[OK] Live yield for {stock['ticker']}: {dividend_data['dividend_yield']*100:.2f}%")
                else:
                    print(f"[WARNING] No live yield for {stock['ticker']}, using static")
                    live_yields.append(stock["yield"] * 100)  # Fallback to static
            except Exception as e:
                print(f"[ERROR] Error getting live yield for {stock['ticker']}: {e}")
                live_yields.append(stock["yield"] * 100)  # Fallback to static
